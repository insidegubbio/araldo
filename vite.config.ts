 ...
 * - POST /__devlogs__/logs: Browser sends logs, written directly to files
...
function vitePluginDebugCollector(): Plugin {
  return {
        ...
        tags: [
          {
            tag: "script",
            attrs: {
              src: "/__devlogs__/debug-collector.js",
              defer: true,
            },
            injectTo: "head",
          },
    ...
    configureServer(server: ViteDevServer) {
      // POST /__devlogs__/logs: Browser sends logs (written directly to files)
      server.middlewares.use("/__devlogs__/logs", (req, res, next) => {
        if (req.method !== "POST") {
          return next();
        }
